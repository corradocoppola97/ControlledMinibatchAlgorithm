...
def average_nfev(architectures, dataset_list, flagtitle):
    ...
    for seed in seeds:
        for ds in dataset_list:
            for architecture in architectures:
                file = 'history_cmal_'+architecture+'_'+ds+'_seed_'+str(seed)+'.txt'
                stats = torch.load(file)
                avg_nfev = stats['nfev']/(len(stats['train_loss'])-1)
                CMAL.append(avg_nfev)

    d['CMA'] = np.array(CMA)
    d['CMAL'] = np.array(CMAL)
    fig = go.Figure()
    fig.add_trace(go.Box(y=d['CMA'], name='CMA',marker_color='blue'))
    fig.add_trace(go.Box(y=d['CMAL'], name='CMAL', marker_color='red'))
    ...
    fig.write_image('images_CMAL\\Avg_nfev_'+flagtitle+ '_' +'.png')
...
def linesearch_failures(architectures, dataset_list, flagtitle):
    ...
    fig.write_image('images_CMAL\\Avg_LSfail_'+flagtitle+'.png')
...
def acceptance_inner_cycle(architectures, dataset_list, flagtitle):
    ...
    fig.write_image('images_CMAL\\Avg_ICacc_'+flagtitle+'.png')
...
def final_step_size(architectures, dataset_list, flagtitle):
    ...
    fig.write_image('images_CMAL\\final_stepsize_'+flagtitle+'.png')
